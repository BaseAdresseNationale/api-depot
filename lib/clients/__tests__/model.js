...
test.serial("create client", async (t) => {
  ...
  await mongo.db
    .collection("mandataires")
    .insertOne({ _id: mandataireId, nom: "mandataire" });

  const chefDeFileId = new mongo.ObjectId();
  await mongo.db
    .collection("chefs_de_file")
    .insertOne({ _id: chefDeFileId, nom: "chef-de-file" });
...
test.serial("update client", async (t) => {
  ...
  const mandataire = await mongo.db
    .collection("mandataires")
    .findOne({ _id: mandataireId });
  const chefDeFile = await mongo.db
    .collection("chefs_de_file")
...
test.serial("computePublicClient", async (t) => {
  ...
  const exposedFields = ["nom", "id", "_id", "mandataire"];
  const exposedClient = await Client.computePublicClient(_id);

  t.true(exposedFields.every((k) => k in exposedClient));
...
test.serial("computePublicClient / with chef de file", async (t) => {
  ...
  const exposedFields = ["nom", "id", "_id", "mandataire", "chefDeFile"];
  const exposedClient = await Client.computePublicClient(_id);
