...
test.serial(
  "lock / Échoue lorsqu’une commune est déjà verrouillée",
  async (t) => {
    const code = "123";
...
test.serial("unlock / Déverrouille une commune avec succès", async (t) => {
  ...
  const unlockedCommune = await mongo.db
    .collection("communes")
    .findOne({ code });
...
test.serial(
  "unlock / Déverrouille une commune déjà déverrouillée sans erreur",
  async (t) => {
    const code = "123";
...
test.serial(
  "unlockAndCleanOverdueCommunes / Déverrouille uniquement les communes dont le verrou est actif depuis trop longtemps",
  async (t) => {
    const now = new Date();
    const overdue = sub(now, { minutes: 5 });
    ...
    const lockedCommune = await mongo.db
      .collection("communes")
      .findOne({ code: "123" });
    t.true(lockedCommune.publishing);
    t.truthy(lockedCommune.publishingSince);
