...
test('create an habilitation', async t => {
  ...
  const keys = ['_id', 'codeCommune', 'emailCommune', 'context', 'strategy', 'client', 'status', 'createdAt', 'updatedAt', 'expiresAt']
...
test('Ask for an habilitation', async t => {
  ...
  const habilitation = {_id, updatedAt: now, createAt: now}
  ...
  const activeHabilitation = await Habilitation.askHabilitation(habilitation, strategy)
  t.is(activeHabilitation.status, 'active')
  t.not(activeHabilitation.updatedAt, now)
  t.truthy(activeHabilitation.strategy)
})
...
test('Validate habilitation', async t => {
  ...
  t.is(validatedHabilitation.status, 'validate')
