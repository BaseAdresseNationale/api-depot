...
test('create an habilitation', async t => {
  ...
  const keys = ['_id', 'codeCommune', 'emailCommune', 'franceconnectAuthenticationUrl', 'strategy', 'client', 'status', 'createdAt', 'updatedAt', 'expiresAt']
...
test('Ask for an habilitation', async t => {
  ...
  const habilitation = {_id, updatedAt: now, createdAt: now, status: 'pending'}
...
test('Accept habilitation', async t => {
  ...
  const acceptedHabilitation = await mongo.db.collection('habilitations').findOne({_id})
...
test('Decrease remaining attempts', async t => {
  ...
  const updatedHabilitation = await decreasesRemainingAttempts(habilitation._id)
