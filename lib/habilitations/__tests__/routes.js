...
test('basic habilitation', async t => {
  ...
  await request(server)
    .post(`/habilitations/${habilitationId}/send-pin-code`)
    .set('Authorization', 'Token foobar')
    .expect(200)

  const habilitation = await mongo.db.collection('habilitations').findOne({_id: mongo.parseObjectID(habilitationId)})
  t.is(habilitation.status, 'active')
  ...
  const res2 = await request(server)
    .post(`/habilitations/${habilitationId}/validate-pin-code`)
    .set('Authorization', 'Token foobar')
    .send({code: habilitation.strategy.pinCode})
    .expect(200)

  t.is(res2.body.status, 'validate')
