...
async function getCommuneEmail(codeCommune) {
  try {
    const url = `${API_ANNUAIRE}/catalog/datasets/api-lannuaire-administration/records?where=pivot%20LIKE%20"mairie"%20AND%20code_insee_commune="${codeCommune}"&limit=100`
    const response = await got(url, {responseType: 'json'})

    // RECUPERE LA MAIRIE PRINCIPAL
    const mairie = response.body.results
      .find(({nom}) => !normalize(nom).includes('deleguee')) || response.body.results[0]
