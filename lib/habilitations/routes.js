...
async function habilitationsRoutes() {
  ...
  app.post('/communes/:codeCommune/habilitations', ensureCommuneActuelle, authClient, w(async (req, res) => {
  ...
  app.get('/habilitations/:habilitationId', authClient, w(async (req, res) => {
    const habilitationWithPublicClient = await expandWithClient(req.habilitation)
