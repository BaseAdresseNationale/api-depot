...
const {
  ...
  generatePinCode,
  getExpirationDate,
} = require("./strategies/email");
const {
  createFranceConnectStrategy,
  getMandatCommune,
} = require("./strategies/franceconnect");

async function habilitationsRoutes() {
  ...
  app.param(
    ...
    w(async (req, res, next) => {
      ...
      if (!habilitation) {
        throw createError(
          404,
          "L’identifiant de l’habilitation demandé n’existe pas",
        );
      }
  ...
  app.post(
    "/communes/:codeCommune/habilitations",
    ensureCommuneActuelle,
    authClient,
    w(async (req, res) => {
  ...
  app.get(
    "/habilitations/:habilitationId",
    authClient,
    w(async (req, res) => {
      const habilitationWithPublicClient = await expandWithClient(
        req.habilitation,
      );
      res.send(omit(habilitationWithPublicClient, "strategy.pinCode"));
    }),
  );
  ...
  app.post(
    ...
    w(async (req, res) => {
      ...
      const { validated, error, remainingAttempts } = await pinCodeValidation(
        req.body.code,
        req.habilitation,
      );
      ...
      const habilitationWithPublicClient = await expandWithClient(habilitation);
      res.send(habilitationWithPublicClient);
    }),
  );

  app
    .route("/habilitations/:habilitationId/authentication/franceconnect")
    .get((req, res, next) => {
  ...
  app.route("/habilitations/franceconnect/callback").get(
    w((req, res, next) => {
      ...
      passport.authenticate("franceconnect", authenticateOptions)(
        req,
        res,
        next,
      );
    ...
    w(async (req, res) => {
      ...
      if (!habilitation) {
        throw createError(
          500,
          "Cette habilitation n’existe plus. Impossible de poursuivre",
        );
      }
