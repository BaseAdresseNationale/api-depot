...
function createFranceConnectStrategy() {
  const strategy = new Strategy({
    ...
    callbackURL: process.env.API_DEPOT_URL + '/habilitations/franceconnect/callback',
    state: 'foobar',
...
function getMandatCommune(user, codeCommune) {
  ...
  if (mandatCommune) {
    return {nomMarital, nomNaissance, prenom, typeMandat: mandatCommune.fonction || 'conseiller-municipal'}
  }
}
