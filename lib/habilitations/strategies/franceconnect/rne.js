...
function findMandats({
  ...
  const elu = (dateNaissanceIndex[dateNaissance] || []).find(
    (c) =>
      c.sexe === sexe &&
      normalize(c.nomNaissance) === nNomNaissance &&
      nPrenom.startsWith(normalize(c.prenom)),
  );
  ...
  console.log(
    `Nom naissance : ${nomNaissance} (FranceConnect) - ${elu.nomNaissance} (RNE)`,
  );
  console.log(
    `Nom marital : ${nomMarital} (FranceConnect) - ${elu.nomMarital} (RNE)`,
  );
