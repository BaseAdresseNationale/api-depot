...
test.serial('update mandataire', async t => {
  ...
  const mandataire = await Mandataire.update(_id, {nom: 'nouveau nom', email: 'nouveau@mail.fr'})
...
test.serial('update mandataire / extra param', async t => {
  ...
  const mandataire = await Mandataire.update(_id, {foo: 'bar'})
  t.deepEqual(mandataire._updatedAt, now)
  t.falsy(mandataire.foo)
})
