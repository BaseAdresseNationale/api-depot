...
test("ensure habilitation / valid", async (t) => {
  ...
  const validatedHabilitation = await Revisions.getRelatedHabilitation(
    habilitationId,
    {
      codeCommune: "27115",
      client: { _id: clientId },
...
test("ensure habilitation / habilitation not accepted", async (t) => {
  ...
  await t.throwsAsync(
    Revisions.getRelatedHabilitation(pendingHabilitationId, context),
    { message: "L’habilitation fournie n’est pas valide" },
  );
  await t.throwsAsync(
    Revisions.getRelatedHabilitation(rejectedHabilitationId, context),
    { message: "L’habilitation fournie n’est pas valide" },
  );
});
...
test.serial("ensure habilitation / different codeCommune", async (t) => {
  ...
  return t.throwsAsync(
    Revisions.getRelatedHabilitation(habilitationId, context),
    { message: "L’habilitation fournie ne concerne pas cette commune" },
  );
});
...
test("ensure habilitation / different client", async (t) => {
  ...
  return t.throwsAsync(
    Revisions.getRelatedHabilitation(habilitationId, context),
    { message: "L’habilitation fournie ne provient pas du même client" },
  );
});
...
test.serial("getRevisionsPublishedBetweenDate", async (t) => {
  ...
  const res0 = await Revisions.getRevisionsPublishedBetweenDate({
    from: sub(new Date(), { days: 1 }),
    to: new Date(),
  });
  t.is(res0.length, 0);
  const res1 = await Revisions.getRevisionsPublishedBetweenDate({
    from: sub(new Date(), { days: 3 }),
    to: new Date(),
  });
  t.is(res1.length, 1);
  const res2 = await Revisions.getRevisionsPublishedBetweenDate({
    from: sub(new Date(), { days: 6 }),
    to: new Date(),
  });
  t.is(res2.length, 2);
  const res3 = await Revisions.getRevisionsPublishedBetweenDate({
    from: sub(new Date(), { days: 11 }),
    to: new Date(),
  });
  t.is(res3.length, 3);
});
...
test.serial("computeRevision", async (t) => {
  ...
  const balFile = await readFile(join(__dirname, "fixtures", "test-valid.csv"));
...
test.serial("computeRevision with relax mode", async (t) => {
  ...
  const balFile = await readFile(
    join(__dirname, "fixtures", "test-valid-relax.csv"),
  );
  const files = {
    _id: new mongo.ObjectId(),
    revisionId: revision._id,
...
test.serial(
  "getFileData - should fallback to DB storage if S3 is not available",
  async (t) => {
    const balFile = await readFile(
      join(__dirname, "fixtures", "bal-valid.csv"),
    );
    ...
    const Revisions = proxyquire("../model", {
      "../files/s3.service": {
        s3Service: {
          ...
          getS3File: sinon
            .stub()
            .returns(Promise.resolve(Buffer.from(balFile))),
        },
      },
    });
...
test.serial(
  "getFileData - should fallback to DB storage if file integrity check fails",
  async (t) => {
    const balFile = await readFile(
      join(__dirname, "fixtures", "bal-valid.csv"),
    );
