...
test.serial('applyValidateBAL - valid', async t => {
  const balFile = await readFile(join(__dirname, 'fixtures', 'bal-valid.csv'))
  const {validation, rows} = await applyValidateBAL(balFile, '31591', null, {rowsCountValue: '6'})
...
test.serial('applyValidateBAL - not valid', async t => {
  ...
  const {validation} = await applyValidateBAL(balFile, '31591', null, {rowsCountValue: '6'})
...
test.serial('applyValidateBAL - bad perimeter', async t => {
  ...
  const {validation, rows} = await applyValidateBAL(balFile, '31591', client, {rowsCountValue: '6'})
