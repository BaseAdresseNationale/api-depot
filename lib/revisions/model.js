...
async function computeRevision(revision) {
  ...
  await mongo.db
    .collection("revisions")
    .updateOne({ _id: revision._id, status: "pending" }, { $set: changes });
...
async function getRelatedHabilitation(habilitationId, { client, codeCommune }) {
  ...
  if (habilitation.client.toString() !== client._id.toString()) {
    throw createError(
      412,
      "L’habilitation fournie ne provient pas du même client",
    );
  }
  ...
  if (habilitation.codeCommune !== codeCommune) {
    throw createError(
      412,
      "L’habilitation fournie ne concerne pas cette commune",
    );
  }
...
async function publishRevision(revision, { client, habilitation }) {
  ...
  try {
    ...
    const publicationType =
      removeCurrentResult.matchedCount > 0 ? "update" : "creation";
...
function getCurrentRevision(codeCommune) {
  return mongo.db
    .collection("revisions")
    .findOne({ codeCommune, current: true });
}
...
async function getCurrentRevisions(publishedSince) {
  ...
  const revisions = await mongo.db
    .collection("revisions")
...
async function expandWithClients(revisions) {
  const clients = await mongo.db
    .collection("clients")
    .find({ _id: { $in: revisions.map((r) => r.client) } })
    .toArray();
  const publicClients = await Promise.all(
    clients.map((client) => Client.computePublicClient(client._id)),
  );
