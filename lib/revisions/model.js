...
async function ensureHabilitation(revision, habilitationId) {
  ...
  if (!habilitation) {
    throw createError(404, 'L’habilitation fournie n’est pas connue')
  }

  if (habilitation.status !== 'accepted') {
    throw createError(403, 'L’habilitation fournie n’est pas valide')
  }

  if (habilitation.codeCommune !== revision.codeCommune) {
    throw createError(403, 'L’habilitation fournie ne concerne pas cette commune')
  }

  if (habilitation.client.nom !== revision.client.nom) {
    throw createError(403, 'L’habilitation fournie ne provient pas du même client')
  }
