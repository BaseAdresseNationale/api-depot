...
async function setFile(revision, type, {data, name}) {
  ...
  const {value} = await mongo.db.collection('files').findOneAndDelete(
    {revisionId: revision._id, type}
  )

  if (value) {
    await s3Service.deleteS3File(value._id.toHexString())
  }
...
async function getFileData(fileId) {
  // CHECK ID IN S3
  const fileS3Exist = await s3Service.checkS3FileExists(fileId.toHexString())
  if (!fileS3Exist) {
    throw new Error('Fichier non trouvé sur S3')
  }
