...
async function revisionsRoutes(params = {}) {
  ...
  async function getIdsFromRevision(revision) {
    ...
    for (const adresse of rowsResult) {
      ...
      } = adresse.parsedValues
      adresseArray.push({
        id: uidAdresse,
        clefmd5: `${voieNom} - ${lieuditComplementNom} - ${numero} - ${suffixe} - ${clefInterop}`
      })
    }
  ...
  async function computeDiffBalFiles(arrayAdresseV1, arrayAdresseCurrent) {
    const idsV1 = arrayAdresseV1.map(({id}) => id)
    const idsCurrent = arrayAdresseCurrent.map(({id}) => id)
  ...
  app.get('/revisions/:revisionId/files/bal/diff', w(async (req, res) => {
    ...
    const resultDiff = await computeDiffBalFiles(await getIdsFromRevision(myRevision), await getIdsFromRevision(currentRevision))
