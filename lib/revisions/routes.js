...
const {fetchRevision, getCurrentRevision, getRevisionsByCommune, createRevision, setFile, getFiles, getFileData, publishRevision, computeRevision, getCurrentRevisions, ensureHabilitation} = require('./model')
...
async function revisionsRoutes(params = {}) {
  ...
  app.post('/revisions/:revisionId/publish', authClient, w(async (req, res) => {
    ...
    if (req.body.habilitationId) {
      habilitation = await ensureHabilitation(req.revision, req.body.habilitationId)
    }
  ...
  app.get('/current-revisions', w(async (req, res) => {
    const currentRevisions = await getCurrentRevisions()
