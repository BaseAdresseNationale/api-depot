...
async function revisionsRoutes() {
  ...
  app.param(
    ...
    w(async (req, res, next) => {
      ...
      if (!revision) {
        throw createError(
          404,
          "L’identifiant de révision demandé n’existe pas",
        );
      }
  ...
  app.get(
    ...
    w(async (req, res) => {
      ...
      const revisionWithPublicClient =
        await Revision.expandWithClient(revision);
  ...
  app.get(
    "/communes/:codeCommune/current-revision/files/bal/download",
    w(async (req, res) => {
      const revision = await Revision.getCurrentRevision(req.codeCommune);
  ...
  app.get(
    ...
    w(async (req, res) => {
      ...
      const revisions = await Revision.getRevisionsByCommune(
        req.codeCommune,
        status,
      );
      const revisionsWithPublicClients = await Promise.all(
        revisions.map((r) => Revision.expandWithClient(r)),
      );
  ...
  app.post(
    ...
    w(async (req, res) => {
      if (!req.body.context) {
        throw createError(
          400,
          "Le contenu de la requête est invalide (context absent)",
        );
      }

      if (
        "nomComplet" in req.body.context &&
        !isString(req.body.context.nomComplet)
      ) {
        throw createError(
          400,
          "Le contenu de la requête est invalide (nomComplet)",
        );
      }

      if (
        "organisation" in req.body.context &&
        !isString(req.body.context.organisation)
      ) {
        throw createError(
          400,
          "Le contenu de la requête est invalide (organisation)",
        );
      }

      if (
        "extras" in req.body.context &&
        !isPlainObject(req.body.context.extras)
      ) {
        throw createError(
          400,
          "Le contenu de la requête est invalide (extras)",
        );
      }
  ...
  app.get(
    ...
    w(async (req, res) => {
      ...
      const revisionWithPublicClient = await Revision.expandWithClient(
        req.revision,
      );
  ...
  app.put(
    ...
    w(async (req, res) => {
      ...
      try {
        ...
        console.error(
          `<<FILE>> Impossible d'upload le fichier ${req.revision._id}`,
        );
        console.error(error);
        throw createError(
          500,
          "Une erreur est survenue lors du téléchargement du fichier BAL",
        );
      }
    }),
  );
  ...
  app.get(
    "/revisions/:revisionId/files/bal/download",
    w(async (req, res) => {
  ...
  app.post(
    ...
    w(async (req, res) => {
      ...
      if (req.body.habilitationId) {
        habilitation = await Revision.getRelatedHabilitation(
      ...
      const publishedRevision = await Revision.publishRevision(req.revision, {
        client: req.client,
        habilitation,
      });
      const publishedRevisionWithPublicClient =
        await Revision.expandWithClient(publishedRevision);
  ...
  app.post(
    ...
    w(async (req, res) => {
      ...
      const computedRevisionWithPublicClient =
        await Revision.expandWithClient(computedRevision);
  ...
  app.get(
    ...
    w(async (req, res) => {
      ...
      const currentRevisions =
        await Revision.getCurrentRevisions(publishedSince);
      const currentRevisionsWithPublicClients =
        await Revision.expandWithClients(currentRevisions);
