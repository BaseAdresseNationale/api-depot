...
const {fetchRevision, getCurrentRevision, getRevisionsByCommune, createRevision, setFile, getFiles, getFileData, publishRevision, computeRevision, getCurrentRevisions} = require('./model')
...
async function revisionsRoutes(params = {}) {
  ...
  app.post('/revisions/:revisionId/publish', authClient, w(async (req, res) => {
    ...
    const publishedRevision = await publishRevision(req.revision, {client: req.client})
