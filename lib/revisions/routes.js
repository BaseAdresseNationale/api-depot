...
async function revisionsRoutes(params = {}) {
  ...
  async function getIdsFromRevision(revision, codeCommune) {
    const revisionData = await getDataFile(revision)
    const result = await applyValidateBAL(revisionData, codeCommune)
    const rowsResult = result.rows
    const adresseArray = []
    for (const adresse of rowsResult) {
      adresseArray.push({id: adresse.rawValues.uid_adresse, clefmd5: md5(adresse.rawValues.voie_nom + adresse.rawValues.lieudit_complement_nom + adresse.rawValues.numero + adresse.rawValues.suffixe + adresse.rawValues.clef_interop)})
    }

    return adresseArray
  }

  async function computeDiffBalFiles(arrayAdresseV1, arrayAdresseCurrent) {
    const idsV1 = []
    const idsCurrent = []
    for (const adresse of arrayAdresseV1) {
      idsV1.push(adresse.id)
    }

    for (const adresse of arrayAdresseCurrent) {
      idsCurrent.push(adresse.id)
    }
    ...
    for (const idTested of commonIds) {
      const foundV1 = arrayAdresseV1.find(element => element.id === idTested)
      const foundCurrent = arrayAdresseCurrent.find(element => element.id === idTested)
      if (foundV1.clefmd5 !== foundCurrent.clefmd5) {
        modifiedIds.push(idTested)
      }
    }
    ...
    return {
      ...
      deletedIds,
      countModified: modifiedIds.length,
      modifiedIds
    }
  }
  ...
  app.get('/revisions/:revisionId/files/bal/diff', w(async (req, res) => {
    ...
    const resultDiff = await computeDiffBalFiles(await getIdsFromRevision(myRevision, myRevision.codeCommune), await getIdsFromRevision(currentRevision, myRevision.codeCommune))
