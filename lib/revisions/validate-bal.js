...
async function applyValidateBAL(file, codeCommune, options = {}) {
  ...
  const {uniqueErrors, rows} = await validate(file, {relaxFieldsDetection: relaxMode})
  ...
  return {rows, validation: {valid: isValid, errors: foundErrors}}
}
  ...
  return async (req, res, next) => {
    try {
      ...
      const {rows, validation} = await applyValidateBAL(
        ...
        {rowsCountValue: req.get('X-Rows-Count'), relaxMode: false}
      )
