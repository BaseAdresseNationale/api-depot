...
} = require("@ban-team/validateur-bal/package.json");
...
async function applyValidateBAL(file, codeCommune, client, options = {}) {
  ...
  const { parseOk, parseErrors, profilErrors, rows } = await validate(file, {
    profile: relaxMode ? "1.3-relax" : "1.3",
  });
  ...
  const foundErrors = profilErrors
    .filter(({ level }) => level === "E")
    .map(({ code }) => code);
  const foundWarnings = profilErrors
    .filter(({ level }) => level === "W")
    .map(({ code }) => code);
  const foundInfos = profilErrors
