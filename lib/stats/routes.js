...
async function statsRoutes() {
  ...
  const clientsToMonitor = await Promise.all(
    Object.values(StatsService.clientsToMonitor).map((clientId) =>
      Clients.getClientByLegacyId(clientId),
    ),
  );
  const clientsToMonitorIndex = keyBy(
    clientsToMonitor.filter((client) => Boolean(client)),
    "_id",
  );

  app.get(
    "/stats/firsts-publications",
    ensureIsAdmin,
    w(async (req, res) => {
      ...
      const firstRevisions =
        await Revisions.getFirstRevisionsPublishedByCommune();
      const firstRevisionsWithClient = mapRevisionsWithClient(
        firstRevisions,
        clientsToMonitorIndex,
      );
      const cumulFirstRevisionsByDate =
        StatsService.getCumulFirstRevisionsByDate(
          firstRevisionsWithClient,
          dates,
        );
  ...
  app.get(
    ...
    w(async (req, res) => {
      ...
      const revisionsWithClient = mapRevisionsWithClient(
        revisions,
        clientsToMonitorIndex,
      );
