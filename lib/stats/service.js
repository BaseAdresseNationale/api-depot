...
function getCumulFirstRevisionsByDate(firstRevisions, dates) {
  ...
  for (
    ...
    const dailyCreations = firstRevisions.filter(
      (firstRevision) =>
        compareDesc(firstRevision.publishedAt, dateIterator) === 1,
    );
    cumulFirstRevisionsByDate.push({
      ...
      viaMesAdresses: dailyCreations.filter(
        ({ client }) => client.id === clientsToMonitor.mesAdresses,
      ).length,
      viaMoissonneur: dailyCreations.filter(
        ({ client }) => client.id === clientsToMonitor.moissonneur,
      ).length,
    });
  }
