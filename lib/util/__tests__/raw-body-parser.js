...
const randomFile = readFileSync(path.join(__dirname, 'fixtures', 'random-file'))
const randomFileMD5 = '71e5090e1cca0942129e95c675edbdbb'
const randomFileGz = readFileSync(path.join(__dirname, 'fixtures', 'random-file.gz'))
...
test('rawBodyParser / Content-MD5 value mismatch', async t => {
  ...
  t.deepEqual(body, {code: 400, message: 'La valeur de l’en-tête Content-MD5 ne correspond pas à la signature MD5 du contenu soumis.'})
})
