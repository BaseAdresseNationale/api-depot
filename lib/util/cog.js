...
const communes =
  require("@etalab/decoupage-administratif/data/communes.json").filter((c) =>
    ["commune-actuelle", "arrondissement-municipal"].includes(c.type),
  );

const arrondissements =
  require("@etalab/decoupage-administratif/data/communes.json").filter((c) =>
    ["arrondissement-municipal"].includes(c.type),
  );
...
function getCommuneByArrondissement(codeArrondissement) {
  return (
    arrondissements.find(({ code }) => code === codeArrondissement)?.commune ||
    null
  );
}
