...
function ensureIsAdmin(req, res, next) {
  if (!ADMIN_TOKEN) {
    return next(
      createError(401, "Aucun jeton d’administration n’a été défini"),
    );
  }

  if (req.get("Authorization") !== `Token ${ADMIN_TOKEN}`) {
    return next(
      createError(401, "Vous n’êtes pas autorisé à effectuer cette action"),
    );
  }
...
async function authClient(req, res, next) {
  if (
    ...
    !req.get("Authorization").startsWith("Token ")
  ) {
  ...
  if (!client.active) {
    return next(createError(403, "Le client est actuellement désactivé"));
  }
...
async function authRevision(req, res, next) {
  if (req.revision.client.toString() !== req.client._id.toString()) {
    return next(
      createError(403, "Vous n’êtes pas autorisé à accéder à cette révision"),
    );
  }
