...
async function authRevision(req, res, next) {
  if (req.revision.client._id.toString() !== req.client._id.toString()) {
    return next(createError(403, 'Vous n’êtes pas autorisé à accéder à cette révision'))
  }

  next()
}

module.exports = {ensureCodeCommune, ensureCommuneActuelle, ensureIsAdmin, authClient, authRevision}
