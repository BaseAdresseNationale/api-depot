  ...
  return async (req, res, next) => {
    try {
      ...
      if (
        ...
        return next(
          createError(400, "Type de compression du contenu non supporté."),
        );
      }
      ...
      if (
        req.get("Content-Disposition") &&
        req.get("Content-Disposition").includes("filename")
      ) {
        req.filename = contentDisposition.parse(
          req.get("Content-Disposition"),
        ).parameters.filename;
      }
      ...
      if (error.message === "MaxBufferError: maxBuffer exceeded") {
        return next(createError(413, "Chaque fichier est limité à 10 Mo."));
      }
