...
function validateData(clients) {
  ...
  if (
    !every(
      clients,
      ({ id, nom, mandataire, token, authorizationStrategy }) =>
        id && nom && token && mandataire && authorizationStrategy,
    )
  ) {
    throw new Error(
  ...
  if (mandataires.length !== clientsCount) {
    throw new Error(
      "Le nombre de mandaires ne correspond pas au nombre de clients",
    );
  }

  const chefsDeFile = clients
    .filter((client) => client.chefDeFile)
    .map(({ chefDeFile, chefDeFileEmail, perimetre, signataireCharte }) => {
      if (!chefDeFileEmail) {
      ...
      if (signataireCharte !== false && signataireCharte !== true) {
        console.error(
          `⚠️ Le chef de file ${chefDeFile} n’a pas de champ "signataireCharte" renseigné`,
        );
      }
...
async function main() {
  const clientsYML = yaml.load(
    readFileSync(join(__dirname, "..", "clients-migration.yml"), "utf8"),
  );
...
async function populateClients(data) {
  ...
  const mandataires = unionWith(data.mandataires, isEqual).map(
    (mandataire) => ({
      ...mandataire,
      _id: new ObjectId(),
      _createdAt: now,
  ...
  console.log(
    `Nombre de mandataires fusionnées : ${data.mandataires.length - mandataires.length}`,
  );

  // Fusion des chefs de files
  const chefsDeFile = unionWith(data.chefsDeFile, isEqual).map(
    (chefDeFile) => ({
      ...chefDeFile,
      _id: new ObjectId(),
      _createdAt: now,
  ...
  console.log(
    `Nombre de chefs de file fusionnées : ${data.chefsDeFile.length - chefsDeFile.length}`,
  );
  ...
  const clients = data.clients.map((item) => {
    const mandataire = mandataires.find(
      ({ nom, email }) =>
        nom === item.mandataire && email === item.mandataireEmail,
    );
    ...
    if (item.chefDeFile) {
      const chefDeFile = chefsDeFile.find(
        ({ nom, email }) =>
          nom === item.chefDeFile && email === item.chefDeFileEmail,
      );
...
async function patchRevisionsWithoutClientId() {
  // Patch legacy revisions without cliend id
  await mongo.db
    .collection("revisions")
    .updateMany(
      { "client.nom": "Guichet Adresse" },
      { $set: { "client.id": "guichet-adresse" } },
    );
  await mongo.db
    .collection("revisions")
    .updateMany(
      { "client.nom": "SIGTOPO Colmar Agglomération" },
      { $set: { "client.id": "sigtopo-colmar-agglomeration" } },
    );
  await mongo.db
    .collection("revisions")
    .updateMany(
      { "client.nom": "SIG du Pays de Brest" },
      { $set: { "client.id": "sig-pays-de-brest" } },
    );
}
