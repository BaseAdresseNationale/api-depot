...
async function main() {
  ...
  const revisionsToRevalidate = await mongo.db.collection('revisions')
    .distinct('_id', query)
  ...
  for (const revisionId of revisionsToRevalidate) {
    ...
    const {validation} = await applyValidateBAL(data, revision.codeCommune, {relaxMode})
    ...
    await mongo.db.collection('revisions').updateOne(
      ...
      {$set: {
        validation: {
          valid: true, validatorVersion, errors, warnings, infos, rowsCount
        }
      }}
    )
  }
