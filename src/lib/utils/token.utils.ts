...
function base62IntToChar(n: number): string {
...
function base62IntToString(n: number): string {
...
function generateBase62Part(): string {
  return base62IntToString(
    Number.parseInt(Math.random().toString().slice(2, 17), 10),
  ).padStart(10, '0');
}

export function generateBase62String(length: number = TOKEN_LENGTH): string {
  let result: string = '';
